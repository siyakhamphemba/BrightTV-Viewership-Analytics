...
3. What content should be recommended on low consumption days?
4. What initiatives would grow BrightTV's user base?
...
Inspected both raw tables, validated the join key, and checked for data quality issues — including a duplicated `UserID` column in the viewership table and ~175 "ghost" profiles (sessions with no associated demographic data).
...
Built a master analytical view (`bright_tv_master`) in Databricks SQL that joins both tables, applies all cleaning logic above, and extracts date/time dimensions (day, month, year, hour, day of week) needed for trend analysis. See [`sql/BrightTV_Viewership_Analysis_2026.sql`](sql/BrightTV_Viewership_Analysis_2026.sql).

**4. Analysis**
12 analytical queries covering subscriber demographics, peak viewing times, channel performance, and low-consumption day identification — each one feeding directly into the dashboards below.

**5. Visualisation**
Three themed Power BI dashboards, connected to the `bright_tv_master` view:
...
> See [`docs/`](docs/) for the full findings write-up.
...
> See [`docs/`](docs/) for full recommendations tied to each of the four business questions.
